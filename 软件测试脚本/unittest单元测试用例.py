...
if __name__ == "__main__":
    ...
    # # 添加多个测试用例if __name__ == '__main__': #运行指定测试用例方法： # 实例化测试套件 suite = unittest.TestSuite() # 添加测试用例到测试套件当中：添加单个测试用例方法 # suite.addTest(TestSuiteDemo1('test01')) #添加多个测试用例 suite.addTests([TestSuiteDemo2('test01'),TestSuiteDemo2('test02')]) # 实例化test_runner runner = unittest.TextTestRunner() # 使用runner运行测试套件runner.run(suite)
    # suitt.addTests([TestCase1("test_1"), TestCase1("test_case")])
